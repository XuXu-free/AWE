...
class SingleStackFixedLyeNMPCController:
    """
    Single-Stack NMPC Controller with Fixed Lye Flow Rate.

    Only optimizes current (I) and coolant flow (v_c).
    Lye flow rate (v_lye) is fixed to a constant value.
    """

    def __init__(self, dt=60.0, horizon=5, sim_dt=0.2, v_lye_fixed=0.03):
        self.dt = dt
        self.sim_dt = sim_dt
        self.horizon = horizon
        self.v_lye_fixed = v_lye_fixed  # Fixed lye flow rate
    ...
    def _dynamics_step(self, T_s_in_k, T_s_k, T_sep_k, T_c_out_k, I_k, v_c_k, n_gas_k):
        """Dynamics with fixed lye flow rate."""
        ...
        v_lye_k = self.v_lye_fixed  # Use fixed lye flow
    ...
    def _setup_solver(self):
        """Setup NLP solver with 2 control variables: I and v_c."""
        # Decision Variables: At each step k: [I, v_c] -> 2 variables
        self.n_controls = 2
        self.U = ca.MX.sym('U', self.n_controls * self.horizon)

        # Parameters: [T_s_in, T_s, T_sep, T_c_out, n_H2_an, n_liq, n_gas, T_ref, P_ref, I_prev, v_c_prev]
        self.n_params = 7 + 1 + self.horizon + 2
        ...
        for k in range(self.horizon):
            ...
            T_s_in_k, T_s_k, T_sep_k, T_c_out_k, Power_k, V_cell, hto_pct, eta = self._dynamics_step(
                T_s_in_k, T_s_k, T_sep_k, T_c_out_k, I_k, v_c_k, n_gas
            )
        ...
        ubx = []
        for k in range(self.horizon):
            lbx.extend([self.I_min, self.v_c_min])
            ubx.extend([self.I_max, self.v_c_max])
        ...
        c_file = os.path.join(output_dir, 'fixed_lye_nmpc_solver.c')
        dll_file = os.path.join(output_dir, 'fixed_lye_nmpc_solver.dll')

        if os.path.exists(dll_file):
            self.solver = ca.nlpsol('fixed_lye_nmpc_solver', 'ipopt', dll_file, opts)
        else:
            print("Compiling Fixed Lye NMPC solver...")
            solver = ca.nlpsol('fixed_lye_nmpc_solver', 'ipopt', nlp, opts)

            try:
                cwd = os.getcwd()
                os.chdir(output_dir)
                try:
                    solver.generate_dependencies('fixed_lye_nmpc_solver.c')
                    print(f"C code generated to {c_file}")

                    if os.name == 'posix':
                        cmd = "gcc -fPIC -shared -O0 fixed_lye_nmpc_solver.c -o fixed_lye_nmpc_solver.dll"
                    else:
                        cmd = "gcc -shared -O0 fixed_lye_nmpc_solver.c -o fixed_lye_nmpc_solver.dll"
        ...
        for k in range(self.horizon):
            ...
            T_s_in_k, T_s_k, T_sep_k, T_c_out_k, _, _, _, _ = self._dynamics_step(
                T_s_in_k, T_s_k, T_sep_k, T_c_out_k, I_k, v_c_k, n_gas_val
            )
    ...
    def solve_nmpc(self, state_vec, P_ref_vec, T_ref, last_action):
        """Solve NMPC with fixed lye flow."""
        self.T_ref = T_ref
        state_vec = np.asarray(state_vec).flatten()

        last_I = last_action[0]
        last_v_c = last_action[2]  # v_c is now at index 2 in [I, v_lye, v_c]
        ...
        try:
            ...
            print(f"Fixed Lye NMPC Failed: {e}")
            return None

    def get_action(self, state_vec, P_ref_vec, T_ref, last_action):
        """Get action [I, v_lye, v_c] with fixed v_lye."""
        ...
        if u_opt is not None:
            ...
            I_cmd = u0[0]
            v_c_cmd = u0[1]
            v_lye_cmd = self.v_lye_fixed  # Return fixed lye flow
        ...
        return last_I, self.v_lye_fixed, last_v_c
