# -*- coding: utf-8 -*-
"""
美化绘制单槽test数据 - 答辩PPT专用版本
数据文件: model_data_20260319_161737.csv
过滤预热段 (t < 0)
"""
...
def create_widescreen_figure(df, output_path):
    ...
    plt.suptitle('Single-Stack AWE System Test - Model Controller',
...
def create_comprehensive_figure(df, output_path):
    ...
    plt.suptitle('Single-Stack AWE System - Model Controller Performance\n(Warmup Removed)',
...
def main():
    # 文件路径
    input_file = "output/single_stack/test/model_data_20260319_161737.csv"
    ...
    # 1. PPT图1: 功率与温度
    create_ppt_figure1_power_and_temp(df, f"{output_dir}/single_stack_power_temp.png")

    # 2. PPT图2: 电流与流量
    create_ppt_figure2_currents_and_flows(df, f"{output_dir}/single_stack_currents_flows.png")

    # 3. PPT图3: 安全指标
    create_ppt_figure3_safety(df, f"{output_dir}/single_stack_safety.png")

    # 4. 综合图表
    create_comprehensive_figure(df, f"{output_dir}/single_stack_comprehensive.png")

    # 5. 宽屏PPT图表
    create_widescreen_figure(df, f"{output_dir}/single_stack_widescreen.png")

    print(f"\nAll figures saved to: {output_dir}/")
    print("Files generated:")
    print("  - single_stack_power_temp.png      (Power & Temperature)")
    print("  - single_stack_currents_flows.png  (Currents & Flows)")
    print("  - single_stack_safety.png          (Safety Metrics)")
    print("  - single_stack_comprehensive.png   (9-panel comprehensive)")
    print("  - single_stack_widescreen.png      (16:9 widescreen layout)")
