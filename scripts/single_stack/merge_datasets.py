...
def add_missing_prev_actions(df):
    ...
    if 'I_prev' not in df.columns:
        if 'I' in df.columns:
            ...
            print("  Added missing column: I_prev")
    ...
    if 'v_lye_prev' not in df.columns:
        if 'v_lye' in df.columns:
            ...
            print("  Added missing column: v_lye_prev")
...
def merge_datasets(step_dir, wind_dir, output_file, keep_step_time=False, validate=True):
    """
    合并step和wind数据集
    ...
    """
    print("=" * 60)
    print("Dataset Merger for Single-Stack AWE")
    print("=" * 60)
    ...
    if step_dir is not None:
        print(f"\n[1/3] Loading STEP datasets from: {step_dir}")
        ...
        step_dfs = []
        print("\n[1/3] Skipping STEP datasets (not specified)")

    # 加载wind数据
    if wind_dir is not None:
        print(f"\n[2/3] Loading WIND datasets from: {wind_dir}")
        ...
        wind_dfs = []
        print("\n[2/3] Skipping WIND datasets (not specified)")
    ...
    # 验证并对齐列
    print("\n[3/3] Validating and aligning datasets...")
    try:
    ...
    if validate:
        ...
        # 检查必需的训练字段
        required_fields = [
            'T_s_in', 'T_s', 'T_sep', 'T_c_out',
            'n_H2_an', 'n_liq', 'n_gas',
            'T_ref', 'I_prev', 'v_lye_prev', 'v_c_prev',
            'I', 'v_lye', 'v_c'
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description='合并阶跃实验和风电实验数据集'
    )
    parser.add_argument('--step_dir', type=str, default=None,
                        help='阶跃实验数据目录 (默认: output/single_stack/dataset/step/)')
    parser.add_argument('--wind_dir', type=str, default=None,
                        help='风电实验数据目录 (默认: output/single_stack/dataset/wind/)')
    parser.add_argument('--output', type=str, default=None,
                        help='输出文件路径 (默认: output/single_stack/dataset/merged/nmpc_dataset_merged_<timestamp>.csv)')
