"""
直接 monkey-patch project 方法来打印 actual_success 的各个组成部分。
"""
import os
import sys
import numpy as np
...
def analyze():
    ...
    # 直接替换 project 方法中的 actual_success 计算部分
    original_project = projector.project

    def patched_project(self, u_ref, state, u_last=None, verbose=False, active_mask=None):
        import io
        import sys
        import casadi as ca
        ...
        try:
            ...
            info = {
                ...
                '_debug': {
                    ...
                    'control_feasible': control_feasible,
                    'cbf_feasible': cbf_feasible,
                    'slack_feasible': slack_feasible,
    ...
    projector.project = types.MethodType(patched_project, projector)
    ...
    for i in range(steps):
        if i % ctrl_steps == 0:
            ...
            if not success:
                n_failures += 1
                if len(failure_debugs) < 3:
                    failure_debugs.append({
                        't': i * dt / 60,
    ...
    print("\nFirst 3 failure debug details:")
    for d in failure_debugs:
        print(f"\n  t={d['t']:.0f}min")
