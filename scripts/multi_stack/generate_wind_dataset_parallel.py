"""
并行风电数据集生成脚本 - 多槽版本
基于单槽版本修改，支持4-stack系统的多进程并行数据生成
"""
import os
import sys
import numpy as np
    ...
    try:
        for t_w in range(warmup_steps):
            ...
            try:
                ...
                u_opt_matrix = np.tile(
                    np.concatenate([I_prev, v_lye_prev, [v_c_prev]]),
                    (horizon, 1)
                )
...
def process_single_month(args_dict):
    ...
    try:
        ...
        # Initial actions
        I_prev = np.ones(n_stacks) * 2000.0
        v_lye_prev = np.ones(n_stacks) * 0.3
        v_c_prev = 0.0
...
def plot_month_data(df, output_dir, timestamp, month_idx, month_name):
    ...
    ax = axes[0, 1]
    ax.plot(t_arr, df['T_sep'], 'g--', label='Separator', alpha=0.7)
    ax.plot(t_arr, df['T_c_out'], 'c:', label='CW Out', alpha=0.7)
    colors = ['r', 'orange', 'brown', 'pink']
    for i in range(4):
        ax.plot(t_arr, df[f'T_s_{i+1}'], color=colors[i], label=f'Stack {i+1}')
    ...
    colors = ['b', 'g', 'r', 'm']
    for i in range(4):
        ax.plot(t_arr, df[f'I_{i+1}'], color=colors[i], label=f'Stack {i+1}')
    ...
    colors = ['b', 'g', 'r', 'm']
    for i in range(4):
        ax.plot(t_arr, df[f'U_cell_{i+1}'], color=colors[i], label=f'Stack {i+1}')
    ...
    colors = ['b', 'g', 'r', 'm']
    for i in range(4):
        ax.plot(t_arr, df[f'v_lye_{i+1}']*1000, color=colors[i], label=f'Stack {i+1}')
    ...
    colors_n = ['b', 'g', 'r', 'm']
    for i in range(4):
        ax.plot(t_arr, df[f'n_H2_an_{i+1}'], color=colors_n[i], label=f'n_H2_an_{i+1}')
    ax.plot(t_arr, df['n_liq'], 'c-', label='n_liq')
    ax.plot(t_arr, df['n_gas'], 'k-', label='n_gas')
...
def main():
    ...
    output_dir = os.path.abspath(os.path.join(
        ...
        'output', 'multi_stack', 'dataset', 'wind'
    ))
