"""
Benchmark 1024 candidates sampling + JIT batch rollout (with proper warm-up)
"""
...
# Full warm-up for all batch sizes
for num_candidates in [128, 1024]:
    ...
    for k in range(ctrl.horizon):
        x = ctrl.batch_sim.step(x, actions[:, :, k])
    torch.cuda.synchronize()

print("Warm-up complete. Running measurements...")
print(f"{'N':>6} | {'sampling_ms':>10} | {'rollout_ms':>10} | {'total_ms':>10} | {'best_cost':>10}")
print("-" * 60)
...
for num_candidates in [128, 256, 512, 1024]:
    ...
    print(f'{num_candidates:6d} | {sampling_ms:10.1f} | {rollout_ms:10.1f} | {sampling_ms+rollout_ms:10.1f} | {costs_np[best_idx]:10.4f}')
