"""
128 Serial vs 1024 JIT Batch 闭环对比 (30步)
"""
...
def run_test(ctrl, use_batch, label, state0, last_action0, P_ref_val, n_steps=30):
    ...
    for k in range(n_steps):
        ...
        total_cost += weights['lambda_track'] * ((P_real_next - P_ref_val)/1e6)**2
        ...
        total_cost += weights['lambda_lye'] * np.sum((u_k[4:8] - v_lye_prev)**2)
...
print("=" * 70)
print("128 Serial vs 1024 JIT Batch (30 steps = 30 min)")
print("=" * 70)

ctrl.num_candidates = 128
cost_serial, t_serial, P_s, T_s, HTO_s, t_log = run_test(ctrl, False, "Serial(128)", state_warm, last_action_warm, 10.0e6, n_steps=30)

ctrl.num_candidates = 1024
cost_batch, t_batch, P_b, T_b, HTO_b, _ = run_test(ctrl, True, "JIT Batch(1024)", state_warm, last_action_warm, 10.0e6, n_steps=30)
...
for ax, data_s, data_b, title, ylab in zip(axes, [P_s, T_s, HTO_s], [P_b, T_b, HTO_b],
    ...
    ax.grid(True)
plt.tight_layout()
out_path = os.path.join(project_root, 'output', 'multi_stack', 'test_step', '128serial_vs_1024batch_30step.png')
