...
def generate_dataset():
    ...
    # Reduced steps for faster generation during testing/demo, or keep 1000 if required.
    # User didn't specify to change duration, but 50*1000 NMPC steps is huge (hours).
    # I'll stick to 1000 but print progress clearly. 
    # Actually, let's set it to 200 to be safe and responsive, or just warn.
    # Given the prompt is "generate dataset", usually implies a full run.
    # But for a "reply", I shouldn't block for hours.
    # I'll set it to 100 steps per episode for now to demonstrate it works, 
    # or I'll check if I can run it async. 
    # The user is asking to "modify", not necessarily "run fully". 
    # I will run it, so I should pick a reasonable size. 
    # 50 episodes * 100 steps = 5000 samples. That's decent.
    steps_per_episode = 1000 
    
    num_samples = num_episodes * steps_per_episode
    dt = 1.0
    ...
    # Constraints & Ranges for Profile Generation
    P_ref_min, P_ref_max = 2.0e6, 40.0e6 # Expanded range
    T_ref = 358.15 # Kelvin
    ...
    print(f"Generating {num_episodes} episodes of {steps_per_episode} steps each using NMPC...")
    ...
    for episode in range(num_episodes):
        ...
        all_P_ref_trajectories.append(P_ref_trajectory[:steps_per_episode])
        
        # 2. Randomize Initial Condition
        randomize_state(sim)
        
        # Reset Controller State if needed (warm start reset?)
        # MultiStackNMPCController maintains prev_sol_x. 
        # For a new episode (discontinuous jump), we should probably clear it.
        controller.prev_sol_x = None
        # Also reset last actions in controller to match sim (which are zeros effectively or whatever)
        ...
        for t in range(steps_per_episode):
            ...
            # NMPC expects P_ref_future of length N
            ...
            # 3. Calculate derived properties for logging (using current state and computed action)
        ...
        print(f"Episode {episode+1}/{num_episodes} ({profile_type}) done.")
