"""
在 14400s (240 步) 阶跃测试中对比 Serial vs JIT Batch Rollout
先快速 warm-up 到稳态，再分别用两种模式跑 4 小时，比较累计 cost 和轨迹
"""
import os, sys, time
import numpy as np
...
def warm_up(ctrl, P_ref, duration_s=14400):
    """用 JIT batch 快速 warm-up，返回最终状态和 last_action"""
...
def run_long_test(ctrl, use_batch, label, state0, last_action0, P_ref_initial, P_ref_final, duration_s=14400, step_time_s=3600):
    ...
    for k in range(n_steps):
        ...
        total_cost += lambda_track * ((P_real_next - P_ref)/1e6)**2
...
print("快速 warm-up 4h 到稳态...")
state_warm, last_action_warm = warm_up(ctrl, 10.0e6, duration_s=14400)
print(f"Warm-up 完成. T_s_mean={np.mean(state_warm[1:5]):.2f}K, T_sep={state_warm[5]:.2f}K")

print("\n" + "=" * 70)
print("14400s Step Test: Serial vs JIT Batch (warm-up state, step at 3600s)")
print("=" * 70)

cost_serial, t_serial, P_s, T_s, HTO_s, t_log = run_long_test(
    ctrl, False, "Serial   ", state_warm, last_action_warm, 10.0e6, 20.0e6, duration_s=14400, step_time_s=3600)

cost_batch, t_batch, P_b, T_b, HTO_b, _ = run_long_test(
    ctrl, True, "JIT Batch", state_warm, last_action_warm, 10.0e6, 20.0e6, duration_s=14400, step_time_s=3600)
...
ax.axvline(x=60, color='gray', linestyle=':', label='Step')
ax.set_title('Power')
...
plt.tight_layout()
out_path = os.path.join(project_root, 'output', 'multi_stack', 'test_step', 'serial_vs_jit_batch_14400s.png')
