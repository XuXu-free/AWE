"""
128 Serial vs 1024 JIT Batch: 14400s (240步) 闭环 cost 对比
"""
...
T_ref = 353.15
weights = dict(lambda_track=1.2, lambda_temp=0.15, lambda_prod=1e-6,
               lambda_I=0.0002, lambda_lye=25000.0, lambda_c=2500.0)
...
def warm_up(ctrl, P_ref, duration_s=14400):
    ...
    for k in range(n_steps):
        ...
        state = sim.step(np.concatenate([I_cmd, v_lye_cmd, [v_c_cmd]]))
        last_action = [I_cmd, v_lye_cmd, v_c_cmd]
    return state.copy(), [np.copy(last_action[0]), np.copy(last_action[1]), last_action[2]]

def run_long_test(ctrl, use_batch, label, state0, last_action0, P_ref_initial, P_ref_final, duration_s=14400, step_time_s=3600):
    ...
    total_time = 0.0
    n_steps = int(duration_s / 60.0)
    step_idx = int(step_time_s / 60.0)
    P_log, T_log, HTO_log, t_log = [], [], [], []

    for k in range(n_steps):
        t = k * 60.0
        P_ref = P_ref_final if k >= step_idx else P_ref_initial
        P_future = [P_ref] * ctrl.horizon
        ...
        total_cost += weights['lambda_track'] * ((P_real_next - P_ref)/1e6)**2
        total_cost += weights['lambda_temp'] * np.sum((T_s_next - T_ref)**2)
        current_diff_sum = sum((I_k[si] - I_k[sj])**2 for si in range(4) for sj in range(si+1, 4))
        total_cost += weights['lambda_prod'] * current_diff_sum
        total_cost += weights['lambda_I'] * np.sum((I_k - I_prev)**2)
        total_cost += weights['lambda_lye'] * np.sum((v_lye_k - v_lye_prev)**2)
        total_cost += weights['lambda_c'] * ((v_c_k - v_c_prev)**2)
        ...
        if k % 10 == 0:
            ...
            hto_pct = (n_H2_sep_gas * sim.R * state[5]) / (sim.P_sys * sim.V_sep_gas) * 100
...
print("=" * 70)
print("128 Serial vs 1024 JIT Batch (14400s = 240 steps, step at 3600s)")
print("=" * 70)

ctrl.num_candidates = 128
cost_serial, t_serial, P_s, T_s, HTO_s, t_log = run_long_test(
    ctrl, False, "Serial(128)", state_warm, last_action_warm, 10.0e6, 20.0e6)

ctrl.num_candidates = 1024
cost_batch, t_batch, P_b, T_b, HTO_b, _ = run_long_test(
    ctrl, True, "JIT Batch(1024)", state_warm, last_action_warm, 10.0e6, 20.0e6)
...
for ax, data_s, data_b, title, ylab in zip(axes, [P_s, T_s, HTO_s], [P_b, T_b, HTO_b],
                                            ['Power', 'Avg Stack Temperature', 'HTO'],
                                            ['MW', 'C', '%']):
    ax.plot(t_log, data_s, 'b-', label='Serial(128)', linewidth=1.5)
    ax.plot(t_log, data_b, 'r--', label='JIT Batch(1024)', linewidth=1.5)
    ...
    ax.grid(True)
plt.tight_layout()
out_path = os.path.join(project_root, 'output', 'multi_stack', 'test_step', '128serial_vs_1024batch_14400s.png')
