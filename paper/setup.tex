...
\sjtusetup{
  ...
  info = {%
    ...
    % 标题
    %
    zh / title           = {基于安全学习的电解水制氢系统智能控制算法研究},
    en / title           = {Intelligent Control Algorithms for Water Electrolysis Hydrogen Production Systems Based on Safe Learning},
    %
    % 标题页标题
    %   可使用“\\”命令手动控制换行
    %
    ...
    % 关键词
    %
    zh / keywords        = {碱性电解水制氢；多槽系统；安全学习；扩散模型；模型预测控制；投影算子；控制障碍函数；安全约束},
    en / keywords        = {AWE; Multi-Stack Electrolysis; Safe Learning; Diffusion Models; Model Predictive Control; Projection Operator; Control Barrier Function; Safety Constraints},
    %
    % 姓名
    %
    zh / author          = {须安楠},
