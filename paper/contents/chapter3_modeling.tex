...
本节针对AWE系统的HTO非仿射约束与温度约束，建立混合阶CBF/HOCBF联合安全框架：首先回顾CBF与HOCBF的基本概念（3.4.1），随后分别推导HTO约束的二阶HOCBF形式（3.4.2）与温度约束的一阶CBF形式（3.4.3），最后将两类约束统一为多重CBF联合优化问题并给出求解算法（3.4.4）。

\subsection{CBF基本概念}

在将CBF理论应用于AWE系统之前，需先明确一阶CBF与高阶HOCBF的数学定义及其适用边界。一阶CBF要求控制输入直接出现在约束函数的一阶导数中；当该条件不满足或灵敏度不足时，需引入HOCBF以约束高阶导数，从而获得更平滑的安全边界响应。以下依次给出二者的形式化定义，并分析其在本文AWE场景下的适用性。
...
一阶CBF虽然能够保证安全集的前向不变性，但在某些工况下存在局限性：当$\partial \dot{h}/\partial \mathbf{u}$在边界附近灵敏度不足时，为维持$\dot{h} + \gamma h \ge 0$条件，控制器可能产生高频切换的控制量。AWE系统的HTO约束正是这种情况——其非仿射特性使一阶CBF在边界附近易导致电流和流量的剧烈抖动，不利于执行机构寿命与系统稳定运行。
高阶控制障碍函数（High-Order CBF, HOCBF）通过约束高阶导数$\ddot{h}$来解决这一问题，使安全边界具备高阶平滑特性，从而抑制控制响应的高频振荡。

定义HOCBF层级函数：
\begin{equation}
\begin{aligned}
\psi_0(\mathbf{x}) &= h(\mathbf{x}) \\
\psi_1(\mathbf{x},\mathbf{u}) &= \dot{\psi}_0 + \alpha_1(\psi_0) \\
\psi_2(\mathbf{x},\mathbf{u}) &= \dot{\psi}_1 + \alpha_2(\psi_1) \\
&\vdots \\
\psi_r(\mathbf{x},\mathbf{u}) &= \dot{\psi}_{r-1} + \alpha_r(\psi_{r-1}) \ge 0
\end{aligned}
\end{equation}
其中$\alpha_i \in \mathcal{K}$为扩展类$\mathcal{K}$函数，通常取线性形式$\alpha_i(s) = \alpha_i \cdot s$。
...
\psi_2 &= \ddot{h} + \alpha_2 \psi_1 \ge 0
\end{aligned}
\end{equation}
展开得：
\begin{equation}
\ddot{h} + \alpha_2 \dot{h} + \alpha_2 \alpha_1 h \ge 0
\end{equation}
这等价于一个二阶低通滤波器作用于$h$，要求$h$以平滑动态趋近于零边界。

基于上述理论分析，针对本文四槽AWE系统的具体约束特性，可做如下选择：HTO约束虽在一阶导数中显含控制输入（相对度为1），但其非仿射特性使一阶CBF在边界附近易导致控制量高频切换，因此采用二阶HOCBF使安全边界具备高阶平滑特性，抑制控制响应的高频振荡；温度约束的相对度为1，控制输入直接出现在一阶导数中且灵敏度充足，采用一阶CBF即可满足要求。

\subsection{HTO约束的二阶HOCBF推导}

HTO约束是AWE系统安全控制的核心难点之一：其动态涉及气液分离器温度与气相氢含量的耦合，且电流通过法拉第效率以非仿射形式进入约束导数。这使得一阶CBF在边界附近易出现控制量高频切换。因此，本节对HTO约束进行完整的二阶HOCBF推导，明确其李导数结构，为后续联合优化提供可计算的约束条件。
...
二阶HOCBF要求对$\dot{h}_{HTO}$再求一次时间导数。在控制周期$\Delta t$内将控制输入$\mathbf{u}$视为分段恒定，则：
\begin{equation}
  \ddot{h}_{HTO} = \frac{d}{dt}\dot{h}_{HTO} = \nabla_{\mathbf{x}} \dot{h}_{HTO} \cdot f(\mathbf{x}, \mathbf{u})
\end{equation}
将$\dot{h}_{HTO} = -K_{HTO}(x_7 \dot{x}_3 + x_3 \dot{x}_7)$代入并应用乘积法则，可得：
\begin{equation}
  \ddot{h}_{HTO} = -K_{HTO}\left( 2\dot{x}_3\dot{x}_7 + x_7\ddot{x}_3 + x_3\ddot{x}_7 \right)
\end{equation}
其中$\ddot{x}_3$与$\ddot{x}_7$可通过对分离器热平衡与气相氢气动态进一步求导得到，涉及各槽温度变化率$\dot{T}_{s,i}$、流量变化率及法拉第效率对温度的偏导数等项。
上述二阶导数的完整显式表达式涉及大量状态耦合项，手工推导易出错且占用篇幅。因此，实际实现中借助CasADi符号自动微分直接计算$\nabla_{\mathbf{x}}\dot{h}_{HTO}$，再与系统动态$f(\mathbf{x},\mathbf{u})$做矩阵-向量乘积得到$\ddot{h}_{HTO}$，在确保导数精度的同时保持代码简洁性。

据此，定义HOCBF层级函数：
\begin{equation}
  \begin{aligned}
    \psi_0(\mathbf{x}) &= h_{HTO}(\mathbf{x}), \\
    \psi_1(\mathbf{x}, \mathbf{u}) &= \dot{h}_{HTO}(\mathbf{x}, \mathbf{u}) + \alpha_1 \psi_0(\mathbf{x}), \\
    \psi_2(\mathbf{x}, \mathbf{u}) &= \ddot{h}_{HTO}(\mathbf{x}, \mathbf{u}) + \alpha_2 \psi_1(\mathbf{x}, \mathbf{u}) \ge 0
  \end{aligned}
\end{equation}
其中$\alpha_1, \alpha_2 > 0$为设计参数。展开得：
\begin{equation}
  \ddot{h}_{HTO} + \alpha_2 \dot{h}_{HTO} + \alpha_2 \alpha_1 h_{HTO} \ge 0
\end{equation}
这等价于一个二阶低通滤波器作用于$h_{HTO}$，要求$h_{HTO}$以平滑动态趋近于零边界。
$\alpha_1$决定$h_{HTO}$趋近边界的指数衰减速度，$\alpha_2$则限制$\dot{h}_{HTO}$的变化率，防止控制突变。
在实际实现中取$\alpha_1 = 0.8$、$\alpha_2 = 2.0$，对应自然频率$\omega_n = \sqrt{\alpha_1\alpha_2} \approx 1.26$~rad/s与阻尼比$\zeta = 0.5\sqrt{\alpha_2/\alpha_1} \approx 0.79$，提供快速且平滑的边界响应。该参数选取兼顾了响应速度与平滑性，确保HTO在逼近安全边界时不会产生导致执行机构疲劳的控制抖动，从而适配工业电解槽的长期运行需求。
...
\begin{equation}
  \begin{array}{ll}
    ...
    h_3(\mathbf{x}) = 0.02 - K_{HTO} \cdot x_3 \cdot x_7, & \text{（HTO上限，二阶HOCBF）}
  \end{array}
  \label{eq:multi_cbf_set}
\end{equation}

温度约束采用一阶CBF条件，因其控制输入直接出现在一阶导数$\dot{h}_1$、$\dot{h}_2$中（见式~\eqref{eq:h1_dot}与式~\eqref{eq:h2_dot}），相对度为1，满足标准CBF可解性要求。HTO约束则采用二阶HOCBF条件，通过引入二阶李导数$L_f^2 h_{HTO}$与一阶补偿项$\alpha_1 h_{HTO}$，可在气液分离动态响应较慢的工况下提供更强的安全裕度与鲁棒性。

在每个控制时刻，CBF安全滤波器将上述不同阶次的条件同时作为不等式约束，求解以下优化问题：
\begin{equation}
  \begin{aligned}
    ...
    & L_f^2 h_{HTO}(\mathbf{x}, \mathbf{u}) + \alpha_2 \bigl[ L_f h_{HTO}(\mathbf{x}, \mathbf{u}) + \alpha_1 h_{HTO}(\mathbf{x}) \bigr] \ge -s_{HTO} \\
    & \mathbf{u}_{min} \le \mathbf{u} \le \mathbf{u}_{max}
  \end{aligned}
  \label{eq:cbf_optimization}
\end{equation}

式中$\gamma_1, \gamma_2 > 0$为温度CBF的类增益参数，决定安全边界收敛速率，典型取值范围为$0.5 \sim 2.0$；$\alpha_1, \alpha_2 > 0$为HTO-HOCBF的补偿系数，实验中取$\alpha_1 = 1, \alpha_2 = 2$以兼顾响应速度与保守性。$s_i \ge 0$为各约束对应的松弛变量，用于处理极端工况下约束暂时不可行的情形：当系统状态已越过安全边界或控制输入饱和导致CBF条件无法满足时，松弛变量允许约束以可控幅度临时放宽，避免优化问题无可行解而导致控制器失效。$\rho_i > 0$为松弛惩罚系数，取值越大，对约束违反的惩罚越重，松弛量越趋近于零。本文对所有9条约束统一取$\rho_i = 5000$，该数值在数值实验中表现出良好的折中——既能在可行情形下将松弛量抑制在$10^{-4}$量级以下，又能在极端瞬态（如功率骤降导致的HTO急剧上升）中保持优化问题的数值可行性。由于HTO约束包含$u_1 \cdot \eta_F(u_1, x_2)$这一关于电流的非仿射项以及二阶李导数$L_f^2 h_{HTO}$，标准的二次规划（QP）方法不再适用，需采用CasADi/IPOPT等通用非线性规划求解器进行数值求解。
...
\begin{algorithm}[htb]
  ...
  \For{每个控制周期$t = 0, 1, 2, \ldots$}{
    ...
    选取代价最小的动作作为名义动作$\mathbf{u}_{nom} = \mathbf{a}^{(\arg\min_i J^{(i)})}_0$\\
...
综上，本节建立了面向四槽AWE系统的混合阶CBF/HOCBF联合安全框架：温度约束采用一阶CBF，HTO约束采用二阶HOCBF以抑制非仿射特性导致的控制高频抖动，二者统一为逐时刻非线性约束优化问题并通过CasADi/IPOPT求解。该框架为生成模型控制策略提供了严格的前向不变性保证，且单次求解时间低于20~ms，满足实时控制要求。
...
系统初始条件如表~\ref{tab:open_loop_init}所示。

\begin{table}[!htbp]
  \centering
  \caption{开环验证初始条件}
  \label{tab:open_loop_init}
  \begin{tabular}{ccc}
    \toprule
    状态变量 & 数值 & 备注 \\
    \midrule
    电解槽温度 $T_{s1\sim4}$ & 85$^\circ$C（358~K） & 4槽相同 \\
    分离器温度 $T_{\mathrm{sep}}$ & 73$^\circ$C（345~K） & — \\
    换热器出口碱液温度 $T_{s,\mathrm{in}}$ & 72$^\circ$C（345~K） & 近似设定 \\
    冷却水出口温度 $T_{c,\mathrm{out}}$ & 52$^\circ$C（325~K） & — \\
    HTO浓度 & 0.52\% & 气相氢中氧体积分数 \\
    H$_2$物质的量分布 & $n_{\mathrm{gas}}=n_{\mathrm{liq}}=n_{\mathrm{H}_2,\mathrm{an},i}\times4$ & 由HTO反算 \\
    \bottomrule
  \end{tabular}
\end{table}

实验在系统达到热力学稳态后开展，稳态时电解槽温度约77.1$^\circ$C，分离器温度约77.0$^\circ$C，HTO约0.458\%。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/open_loop_response.png}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/controller_step_comparison.png}
  \caption{TCN Diffusion、Diffusion MLP、NMPC与MLP阶跃响应对比。其中，(a) 总功率跟踪；(b) 电解槽温度；(c) 氢氧杂质含量；(d) 电解槽电流；(e) 碱液流量；(f) 冷却水流量。}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/controller_cost_comparison.png}
  \caption{四种控制方法总成本及单步计算时间对比。其中，(a) 各成本分量对比；(b) 单步计算时间对比。}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/controller_wind_comparison.png}
  \caption{TCN Diffusion、Diffusion MLP、NMPC与MLP风电功率跟踪对比。其中，(a) 总功率跟踪；(b) 电解槽温度；(c) 氢氧杂质含量；(d) 电解槽电流；(e) 碱液流量；(f) 冷却水流量。}
...
系统初始运行于3000~A稳态，$t=60$~min时刻参考电流阶跃降至1000~A，模拟可再生能源出力骤降。该工况下产气速率下降而气液分离动态滞后，阳极侧氢气累积导致HTO上升。CBF参数取$\alpha_1=1$、$\alpha_2=2$、$\rho=5000$，与温度CBF联合构成混合阶约束体系。

图~\ref{fig:cbf_hto_protection} 展示了闭环验证结果。HTO子图顶部红色指示条标记了CBF触发时刻。尽管动态滞后导致HTO呈上升趋势，CBF安全滤波通过及时增大碱液流量并适度调节电流，将HTO严格抑制在2\%安全阈值以下（峰值2.06\%）。这一控制分配与式~\eqref{eq:h_hto_expanded} 的物理结构一致：碱液流量以仿射形式直接作用于$\dot{h}_{HTO}$，调节灵敏度最高，故CBF优先通过增大流量快速抑制氢气累积；电流则以非仿射耦合项$\gamma(\mathbf{x}) \cdot u_1 \cdot \eta_F$进入约束，受法拉第效率饱和特性制约，其边际改善有限且伴随功率损失，因此仅被适度调节。为定量验证一阶CBF与二阶HOCBF在相同工况下的控制品质差异，补充了对比实验：保持实验条件完全一致（初始3000~A稳态，$t=60$~min阶跃降至1000~A，$\alpha_1=1$、$\rho=5000$），分别关闭与启用二阶HOCBF机制。
图~\ref{fig:cbf_order_comparison} 展示了对比结果。保持实验条件完全一致（初始3000~A稳态，$t=60$~min阶跃降至1000~A，$\rho=5000$、$h_{\rm margin}=0.001$）：一阶CBF（$\gamma=10$，不使用$u_{\rm weight}$）虽能将HTO峰值抑制在1.95\%，但控制量出现显著高频震荡，$|\Delta I|$均值高达99.7~A，总电流在5--12~kA范围内持续振荡；二阶HOCBF采用混合阶设计——HTO通道取$\alpha_1=10$、$\alpha_2=2$以提供高阶平滑特性，温度通道取$\alpha_1=20$、$\alpha_2=20$近似一阶CBF——并启用$u_{\rm weight}$（电流权重$500$、碱液权重$20$），将HTO峰值严格控制在1.97\%（低于2\%安全限），同时$|\Delta I|$均值降至5.3~A，碱液流量$|\Delta v_{\rm lye}|$均值仅0.04~L/s，控制动作显著平滑，阶跃后的稳态阶段未出现持续高频抖振。
这一现象可由相对度理论解释：一阶CBF仅要求$\dot{h}_{HTO}+\alpha_1 h_{HTO}\geq 0$，其约束对控制量的变化率无直接限制，优化器在约束边界与参考跟踪之间频繁切换，导致控制量高频抖振；二阶HOCBF通过引入辅助函数$\psi_1 = \dot{h}_{HTO} + \alpha_1 h_{HTO}$并约束$\dot{\psi}_1 + \alpha_2 \psi_1 \geq 0$，实质上等价于对控制量的变化率施加了惯性约束，配合$u_{\rm weight}$对参考跟踪的强权重，有效抑制了非仿射特性导致的高频抖动，验证了混合阶框架在控制抖振抑制方面的有效性。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/cbf_order_comparison.png}
  \caption{一阶CBF与二阶HOCBF在HTO约束保护工况下的对比。其中，(a) 电解槽电流；(b) 碱液流量；(c) 冷却水流量；(d) HTO安全控制。}
  \label{fig:cbf_order_comparison}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/cbf_hto_protection.png}
  \caption{多重CBF联合约束闭环验证结果（HTO约束保护工况）。其中，(a) 电解槽电流；(b) 碱液流量；(c) 冷却水流量；(d) HTO安全控制。}
...
图~\ref{fig:cbf_temp_protection} 展示了闭环验证结果。电流阶跃后各槽温度明显上升，CBF在温度接近安全边界时及时触发（共162次，占11.3\%），通过主动增加冷却水流量（从20~L/s增至最高32.5~L/s）并适度调节电流，将温度严格控制在90$^\circ$C边界（最高90.00$^\circ$C）。HTO同步保持在2\%以内（峰值1.06\%）。该控制行为可由式~\eqref{eq:h1_dot} 解释：冷却水流量以仿射形式直接作用于热平衡动态，是降低温度灵敏度最高的控制通道，因此CBF优先大幅增大冷却水以快速带走反应热；电流则以非仿射项$\alpha(\mathbf{x}, u_1)$进入约束，且由$U_{cell} > \eta_F U_{th}$可知增大电流恒导致产热增加，故CBF仅适度调节电流以避免热负荷进一步恶化。温度约束采用一阶CBF即可获得平滑且迅速的冷却水响应，无需引入二阶HOCBF，验证了3.4.2.4节混合阶策略的物理合理性。

综合两个场景可见：低功率工况下HTO约束主导安全边界，二阶HOCBF通过碱液流量的平滑调节抑制氢气累积，避免了非仿射特性导致的高频抖动；高功率工况下温度约束主导，一阶CBF通过冷却水流量的直接快速响应控制热平衡，因其控制输入灵敏度充足无需引入高阶惯性。两种极端工况分别验证了混合阶CBF/HOCBF框架在不同相对度约束下的安全保证能力，且单次求解均满足实时控制要求。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/cbf_temp_protection.png}
  \caption{多重CBF联合约束闭环验证结果（高温约束保护工况）。其中，(a) 电解槽电流；(b) 碱液流量；(c) 冷却水流量；(d) 温度安全控制。}
