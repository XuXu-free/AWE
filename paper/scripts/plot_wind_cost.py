...
min_len_paper = min(
    pd.read_csv(base_dir + "/" + f).shape[0] - (warmup_rows if 'nmpc' not in f else 0)
    for f in paper_models.values()
)
paper_costs = {}
for name, f in paper_models.items():
    skip = warmup_rows if 'nmpc' not in f else 0
    paper_costs[name] = compute_cost(base_dir + "/" + f, skip_warmup=skip, max_len=min_len_paper)
...
ax.set_ylabel('成本')
ax.set_title('(a) 各成本分量对比', fontsize=14)
...
ax.set_ylabel('总成本')
ax.set_title('(b) 总成本对比', fontsize=14)
setup_ax(ax)
for bar in bars:
...
for name, f in internal_models.items():
    skip = warmup_rows if 'nmpc' not in f else 0
    internal_costs[name] = compute_cost(base_dir + "/" + f, skip_warmup=skip, max_len=min_len_internal)
...
plt.tight_layout()
plt.savefig('../../output/multi_stack/test/run_20260426_121806/all_models_wind_cost_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
